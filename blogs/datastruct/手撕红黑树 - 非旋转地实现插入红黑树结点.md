...
```py
    def _rotate(self, parent, left, mid, right, c1, c2, c3, c4):
        """
        按照位置关系将7个结点连接起来即可
        """
...
class RBTreeMap:
    ...
    def _rebalance(self, x):
        ...
        if px == ppx.left:
            if x == px.left:
                mid = self._rotate(parent, x, px, ppx, x.left, x.right, px.right, ppx.right)
            else:
                mid = self._rotate(parent, px, x, ppx, px.left, x.left, x.right, ppx.right)
        else:
            if x == px.right:
                mid = self._rotate(parent, ppx, px, x, ppx.left, px.left, x.left, x.right)
            else:
                mid = self._rotate(parent, ppx, x, px, ppx.left, x.left, x.right, px.right)
