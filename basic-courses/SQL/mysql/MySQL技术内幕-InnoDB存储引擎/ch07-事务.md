...
- 需要上锁(`prepare_commit_mutex`), 而上锁则导致commit只能串行, group commit失效
...
1. prepare 阶段: 
    1. 引擎层: 
2. flush 阶段:  
    1. 引擎层: flush多个事务的redo log, 并一次性fsync (group commit)
...
- 场景:
    ```
    L1     tx A: begin;                           
    L2     tx A: delete * from t where a <= 5;    // 写入undo log
    L3     tx B: begin;                           
    L4     tx B: insert into t select 3;          // 写入undo log
    L5     tx B: commit;                          // 提交, B先写入bin log
    L6     tx A: commit;                          // A再写入bin log
    ```
- 解读
    - A的事务先开启, 事务号比B小, 因此purge时先处理A, 再处理B
    - 由于MVCC, B的undo log中看不到A插入的记录, 因此undo log中只记录了对a=3以外记录的删除. 
        - 因此两个事务都提交后, 主机上B事务insert的记录还在
    - 写入bin log的顺序是B, A, 从机先执行B, 再执行A, 即先插入, 后删除, 
        - 因此从机中B插入的数据就被删了