...
- 创建表/扩容(-1): 线程通过`CAS(sizeCtl, old, -1)`竞争建表机会, 结束后再改成新的扩容阈值
- 存在表(>0): 表示扩容阈值, 其中`Integer.MAX_VALUE`表示无法再扩容
- transfer(<0): 迁移过程中, 高16位用来表示新表的size(也表示扩容的epoch), 低16用来表示transfer线程计数器
...
- 与1.7的区别: 
    - 1.7只有一个需要竞争的共享变量对象(segments), 而1.8有两个竞争对象(sizeCtl, table)
